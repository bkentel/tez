...
void item_parser::rule_root(cref json_value) {
    try {
        ...
        auto item_list = json::require_key(json_value, bklib::string_ref{"items"});
...
void item_parser::rule_item_list(cref json_value) {
    try  {
        ...
        json::for_each_element_skip_on_fail(json_value, [&](cref item) {
            rule_item(item);
            items_.emplace(item_ref{item_.id.hash}, std::move(item_));
        });
    } catch (json::error::base& e) {
        BK_JSON_ADD_TRACE(e);
    }
...
void item_parser::rule_item(cref json_value) {
    try  {
        auto item_id        = json::require_key(json_value, bklib::string_ref{"id"});
        auto item_category  = json::require_key(json_value, bklib::string_ref{"category"});
        auto item_type      = json::require_key(json_value, bklib::string_ref{"type"});
        auto item_tag_list  = json::require_key(json_value, bklib::string_ref{"tags"});
        auto item_base_val  = json::require_key(json_value, bklib::string_ref{"base_value"});
        auto item_attr_list = json::require_key(json_value, bklib::string_ref{"attributes"});
        auto item_name      = json::require_key(json_value, bklib::string_ref{"name"});
        auto item_desc      = json::require_key(json_value, bklib::string_ref{"description"});
...
void item_parser::rule_item_attr(cref json_value) {
    try  {
        ...
        auto name  = json::require_key(json_value, 0u);
        auto value = json::require_key(json_value, 1u);
...
using map_t = boost::container::flat_map<Key, T>;

map_t<tez::item_ref, tez::item_definition> items_map;

} //namespace local_state
} //namespace

void tez::reload_items(bklib::utf8string filename) {
    item_parser parser {filename};
    parser.parse();
    local_state::items_map = parser.get();
}

item_definition const* tez::get_item(hash_wrapper<detail::tag_item> ref) {
    auto const where = local_state::items_map.find(item_ref{ref.value});

    return (where != std::cend(local_state::items_map))
      ? std::addressof(where->second)
      : nullptr
    ;
}
